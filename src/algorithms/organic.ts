...
export function addOrganicGeometry (sentence: Sentence): void {
  ...
  const boundaryPoints = range(0, boundaryResolution).map(index => {
    const angle = index * 2 * Math.PI / boundaryResolution - Math.PI / 2
    return [
  ...
  boundaryPoints.forEach((thisPoint, index) => {
    const nextIndex = index === boundaryResolution - 1 ? 0 : index + 1
